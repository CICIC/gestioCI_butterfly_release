...
class period_close_form(forms.ModelForm):
	...
	def __init__(self, *args, **kwargs):
		...
		if self.is_new:
			...
			if self.base_fields["record_type"].initial is None: #protect against multiple call, if we are loaded don't load again
	...
	def get_vat_type_display(self):
		from Finances.models import vat_TYPES
	...
	class Meta:
		...
		exclude = ()
from Finances.models import period_payment
